...
- **Session validation**: Three-tier model. **Tier 1 HARD BLOCKS (9 code-definition rules):** (1) same provider overlapping 1:1 sessions, (2) same client overlapping incompatible sessions (with code-pair exemptions for supervision + caregiver training), (3) RBT billing QHP-only codes, (4) same provider 97153+97155 overlap, (5) group codes need 2-8 clients, (6) 97158 must be QHP, (7) auth utilization >100%, (8) 97151/97152 assessment codes exclusive (no concurrent sessions), (9) provider can't be their own supervisor. **Tier 2 CONFIGURABLE WARNINGS (AMA/ABA defaults, payer-overridable):** concurrent billing code pairs, MUE limits, auth date range, missing supervisor, credential expiring. Warning includes "save as payer rule" option. **Tier 3 INFORMATIONAL.** Only code DEFINITIONS are universal — everything else varies by payer. See CEO plan "Validation Philosophy Amendment" and `docs/research/scheduling/` for full research.
