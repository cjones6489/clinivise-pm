...
const clients = [
  { id: "cl_my_01", first: "Ethan", last: "Miller", dob: "2018-03-15", gender: "M" as const, guardian: "Rebecca Miller", payerId: PAYER_BCBS, rbt: PROV_RBT1 },
  { id: "cl_my_02", first: "Sophia", last: "Garcia", dob: "2019-07-22", gender: "F" as const, guardian: "Maria Garcia", payerId: PAYER_AETNA, rbt: PROV_RBT2 },
  { id: "cl_my_03", first: "Liam", last: "Johnson", dob: "2017-11-03", gender: "M" as const, guardian: "Tanya Johnson", payerId: PAYER_UHC, rbt: PROV_RBT1 },
  { id: "cl_my_04", first: "Olivia", last: "Thompson", dob: "2020-01-18", gender: "F" as const, guardian: "Marcus Thompson", payerId: PAYER_MEDICAID, rbt: PROV_RBT2 },
  { id: "cl_my_05", first: "Noah", last: "Williams", dob: "2018-09-05", gender: "M" as const, guardian: "Jordan Williams", payerId: PAYER_BCBS, rbt: PROV_RBT1 },
  { id: "cl_my_06", first: "Emma", last: "Rodriguez", dob: "2019-04-12", gender: "F" as const, guardian: "Carlos Rodriguez", payerId: PAYER_AETNA, rbt: PROV_RBT2 },
  { id: "cl_my_07", first: "Aiden", last: "Chen", dob: "2016-12-28", gender: "M" as const, guardian: "Lisa Chen", payerId: PAYER_UHC, rbt: PROV_RBT1 },
  { id: "cl_my_08", first: "Isabella", last: "Patel", dob: "2020-06-09", gender: "F" as const, guardian: "Priya Patel", payerId: PAYER_MEDICAID, rbt: PROV_RBT2 },
];

// Auth profiles — mix of states
const authProfiles = [
  { startOff: -120, endOff: 60, usedPct: 0.55, status: "approved" },  // active, healthy
  { startOff: -90, endOff: 90, usedPct: 0.40, status: "approved" },   // active, healthy
  { startOff: -150, endOff: 30, usedPct: 0.70, status: "approved" },  // active, moderate
...
async function seed() {
  ...
  await db
    ...
    .set({ name: "Bright Futures ABA", npi: "1234567893", taxId: "74-1234567", taxonomyCode: "103K00000X", phone: "(512) 555-0100", addressLine1: "100 Congress Ave, Suite 200", city: "Austin", state: "TX", zipCode: "78701" })
    .where(eq(schema.organizations.id, ORG_ID));

  // Providers
  console.log("  Providers...");
  await db.insert(schema.providers).values([
    { id: PROV_BCBA, organizationId: ORG_ID, userId: USER_OWNER_ID, firstName: "Sarah", lastName: "Chen", credentialType: "bcba_d", npi: "1111111112", credentialNumber: "BCBA-D-12345", credentialExpiry: relDate(365), modifierCode: "HP" },
    { id: PROV_BCBA2, organizationId: ORG_ID, firstName: "Marcus", lastName: "Wright", credentialType: "bcba", npi: "2222222223", credentialNumber: "BCBA-67890", credentialExpiry: relDate(200), supervisorId: PROV_BCBA, modifierCode: "HO" },
    { id: PROV_RBT1, organizationId: ORG_ID, firstName: "David", lastName: "Park", credentialType: "rbt", credentialNumber: "RBT-11111", credentialExpiry: relDate(180), supervisorId: PROV_BCBA, modifierCode: "HM" },
    { id: PROV_RBT2, organizationId: ORG_ID, firstName: "Jessica", lastName: "Torres", credentialType: "rbt", credentialNumber: "RBT-22222", credentialExpiry: relDate(90), supervisorId: PROV_BCBA2, modifierCode: "HM" },
  ]).onConflictDoNothing();

  // Payers
  console.log("  Payers...");
  await db.insert(schema.payers).values([
    { id: PAYER_BCBS, organizationId: ORG_ID, name: "Blue Cross Blue Shield of Texas", payerType: "commercial", timelyFilingDays: 90, unitCalcMethod: "ama" },
    { id: PAYER_AETNA, organizationId: ORG_ID, name: "Aetna", payerType: "commercial", timelyFilingDays: 90, unitCalcMethod: "ama" },
    { id: PAYER_UHC, organizationId: ORG_ID, name: "UnitedHealthcare", payerType: "commercial", timelyFilingDays: 120, unitCalcMethod: "ama" },
    { id: PAYER_MEDICAID, organizationId: ORG_ID, name: "Texas Medicaid (HHSC)", payerType: "medicaid", timelyFilingDays: 365, unitCalcMethod: "cms" },
  ]).onConflictDoNothing();
  ...
  for (const c of clients) {
    ...
    // Insurance
    await db.insert(schema.clientInsurance).values({
      id: `ins_my_${c.id}`,
      organizationId: ORG_ID,
      clientId: c.id,
      payerId: c.payerId,
  ...
  for (let i = 0; i < clients.length; i++) {
    ...
    await db.insert(schema.authorizationServices).values([
      {
